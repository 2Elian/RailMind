...
def debug_database():
    ...
    samples = kg_system.run_query(sample_query)
    for i, sample in enumerate(samples, 1):
        print(f"\n列车 {i}:")
        print(f"  车次: {sample['车次']}")
        print(f"  路线: {sample['始发站']} → {sample['终到站']}")
        print(f"  时间: {sample['发车时间']} - {sample['到达时间']}")
        if sample['候车厅']:
            print(f"  候车厅: {', '.join(sample['候车厅'])}")
        if sample['检票口']:
            print(f"  检票口: {', '.join(sample['检票口'])}")
        if sample['站台']:
            print(f"  站台: {', '.join(sample['站台'])}")
    
    # 2. 检查车站名称的具体格式
    print("\n2. 车站名称格式检查:")
    station_format_query = """
    MATCH (s:Station)
    RETURN DISTINCT s.station_name as station_name
    ORDER BY s.station_name
    LIMIT 20
    """
    
    stations = kg_system.run_query(station_format_query)
    print(f"前20个车站名称:")
    for station in stations:
        name = station['station_name']
        print(f"  '{name}' (长度: {len(name)}, 类型: {type(name).__name__})")
    ...
    RETURN 
        ...
        t.departure_time as 发车时间,
        t.arrival_time as 到达时间,
        TYPE(t.departure_time) as 发车时间类型,
        TYPE(t.arrival_time) as 到达时间类型
    LIMIT 5
    """
    
    times = kg_system.run_query(time_format_query)
    for time_data in times:
        print(f"  {time_data['车次']}: {time_data['发车时间']} ({time_data['发车时间类型']})"
              f" → {time_data['到达时间']} ({time_data['到达时间类型']})")
    
    # 4. 检查时间比较
    print("\n4. 时间比较测试:")
    time_test_query = """
    MATCH (t:Train)
    WHERE t.departure_time >= '08:00' AND t.departure_time <= '12:00'
    RETURN COUNT(t) as 时间段内列车数量
    """
    
    time_test = kg_system.run_query(time_test_query)
    print(f"08:00-12:00时间段的列车数量: {time_test[0]['时间段内列车数量'] if time_test else 0}")
    
    # 5. 完整统计
    print("\n5. 数据库统计:")
    stats_query = """
    MATCH (t:Train)
    WITH COUNT(t) as total_trains
    
    MATCH (t:Train)-[:DEPARTS_FROM]->(dep:Station)
    WITH total_trains, COUNT(DISTINCT dep) as departure_stations
    
    MATCH (t:Train)-[:ARRIVES_AT]->(arr:Station)
    RETURN 
        total_trains as 列车总数,
        departure_stations as 始发站数量,
        COUNT(DISTINCT arr) as 到达站数量
    """
    
    stats = kg_system.run_query(stats_query)
    if stats:
        print(f"  列车总数: {stats[0]['列车总数']}")
        print(f"  始发站数量: {stats[0]['始发站数量']}")
        print(f"  到达站数量: {stats[0]['到达站数量']}")
